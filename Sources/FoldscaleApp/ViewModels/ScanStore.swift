...
/// The bridge between `FoldscaleCore` and the UI: it owns the current scan, streams
...
final class ScanStore {
    ...
    private static let persistQueue = DispatchQueue(label: "io.github.mikehoncho32.foldscale.persist")
    ...
    nonisolated static func migrateLegacyPreferences() {
        ...
        guard defaults.object(forKey: autoRefreshDefaultsKey) == nil,
            let legacy = UserDefaults(suiteName: "io.github.mikehoncho32.radix"),
            let value = legacy.object(forKey: autoRefreshDefaultsKey) as? Bool
        else { return }
        defaults.set(value, forKey: autoRefreshDefaultsKey)
    }
    ...
    private let protectedRoots: [URL] = {
        ...
        if let support {
            roots.append(support.appendingPathComponent("Foldscale"))
        }
        return roots
    }()
    ...
    var rootDisplayName: String {
        guard let rootURL else { return "Foldscale" }
        let volumeName =
    ...
    /// Demo mode (`FOLDSCALE_DEMO=1`): a hand-written tree stands in for a scan so
    ...
    private func logIfRequested(_ tree: FileTree) {
        guard ProcessInfo.processInfo.environment["FOLDSCALE_LOG"] != nil else { return }
        let root = tree.rootID
        let line =
            "FOLDSCALE_SCAN_DONE count=\(tree.count) items=\(tree.itemCount(of: root)) "
