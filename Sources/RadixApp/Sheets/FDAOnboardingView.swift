...
struct FDAOnboardingView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text(
                "Radix reads file sizes across your disk. macOS keeps some folders — Mail, "
                    + "Messages, Time Machine, other apps' data — behind Full Disk Access, so parts "
                    + "of a scan can come back incomplete without it. Radix has no network access."
            )
            .fixedSize(horizontal: false, vertical: true)

            Text("Open the setting below, enable Radix in the list, then return here.")
