...
/// Stages the window for screenshots when `RADIX_DEMO` is set: a fixed window size
/// (`RADIX_WINDOW=1280x800`), an explicit appearance (`RADIX_APPEARANCE=dark|light`),
/// a destination (`RADIX_DEMO_VIEW=home|drive|freeup|<SmartListKind raw value>`) and
/// outline rows to expand (`RADIX_DEMO_EXPAND=1,3`, zero-based, in the focused folder).
@MainActor
enum DemoStaging {
    static var isEnabled: Bool { ProcessInfo.processInfo.environment["RADIX_DEMO"] != nil }

    static func apply(store: ScanStore, bridge: OutlineBridge, select: @escaping (SidebarItem?) -> Void) {
        let env = ProcessInfo.processInfo.environment
        if let appearance = env["RADIX_APPEARANCE"] {
            NSApp.appearance = NSAppearance(named: appearance == "dark" ? .darkAqua : .aqua)
        }
        let size = parseSize(env["RADIX_WINDOW"]) ?? NSSize(width: 1280, height: 800)
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            if let view = env["RADIX_DEMO_VIEW"], let item = destination(view, store: store) { select(item) }
        }
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.8) {
            guard let outline = bridge.outline, let rows = env["RADIX_DEMO_EXPAND"] else { return }
