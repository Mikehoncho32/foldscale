...
final class ScannerTests: XCTestCase {
    ...
    func testTotalAllocatedMatchesDu() throws {
        ...
        XCTAssertEqual(
            ...
            "FoldscaleCore's aggregated allocated size must match `du` exactly"
        )
    }
    ...
    func testRootNotFound() {
        let missing = URL(fileURLWithPath: "/nope/does/not/exist/foldscale-\(UUID().uuidString)")
    ...
    private func makeTempDir() throws -> URL {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("foldscale-test-\(UUID().uuidString)")
